...
`receipt-items--view`. Элементы строки (`.receipt-item-row`,
`.receipt-item-number`, `.receipt-item-product`, `.receipt-item-field` и т.д.) —
общие; различаются сетка и наличие колонки действий.
...
распределение колонок и не смещались относительно друг друга.

На мобильном редактор перестраивается в карточку: номер и товар в одной строке,
под ними поля, ниже — две кнопки действия на всю ширину (`icon-button` +
`icon-button__label`). Мобильная раскладка переключается на существующем
брейкпоинте `max-width: 768px`.
