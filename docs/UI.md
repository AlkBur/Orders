...
Исключение — карточка пользователя: она дополнительно содержит действие
«Удалить» (danger-кнопка), так как управление пользователями полностью
административное. Действие доступно только администратору, требует
подтверждения (`data-confirm`) и выполняется `POST /users/{id}/delete`.
Последнего администратора удалить нельзя: на его карточке кнопка не
показывается, а сервер повторно проверяет инвариант (`ErrLastAdministrator`).
Остальные карточки по-прежнему не содержат служебных кнопок.
...
Количество, цену и сумму строки можно редактировать и прямо в таблице
редактора (без открытия модалки). Ограничения одинаковы для обоих способов:
значения должны быть больше нуля, а документ без строк не сохраняется.
Проверка выполняется на клиенте (подсветка полей `is-danger` после выхода из
поля или попытки сохранения и блокировка отправки через `htmx:configRequest`)
и повторно на сервере.
...
Числовые показатели имеют разную точность: **количество — до 3 знаков**
после запятой (`step="0.001"`, при вводе третий и последующие знаки
урезаются), **цена и сумма — до 2 знаков** (`step="0.01"`, третий знак при
вводе урежается). Реактивные вычисления округлены: сумма
`round2(количество × цена)`, цена при ручном вводе суммы
`round2(сумма / количество)`, итог — сумма округлённых `amount` строк.
...
- подтверждение реализуется атрибутом `data-confirm` на этой форме,
  обработчиком в `static/js/confirm.js`, который показывает модальное окно
  подтверждения (заголовок «Подтверждение», кнопки «ОК»/«Отмена»)
  — общий механизм для карточек и списков;
...
### Журнал товарных чеков

Специализированный список (`templates/list/page.html`, `ReceiptsListPage`)
отображает колонки: Номер, Дата, Организация, Пользователь (логин
создателя), Клиент, Сумма, Статус, Инфо, Действия.

Панель расширенного отбора (`FilterData`) поддерживает период, сумму,
организацию, контрагента, пользователя-создателя, статус и дату установки
действия. Значения фильтра приходят только из query parameters; отбор по
пользователю — серверный `<select name="user_id">` (без Alpine).

У документа со статусом «Отменен» кнопка «Файлы» в списке не
показывается; в карточке просмотра имена файлов выводятся без ссылок, а
прямой запрос файлов возвращает `403`.

Карточка просмотра показывает дату документа со временем создания
(`created_at`), а после отправки — дополнительную строку «Отправлен в 1С»
с датой и временем `SentAt`.
