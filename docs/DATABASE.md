...
| status | TEXT | Статус (управляется 1С). Фиксированный список: `Создан`, `Отправлен`, `Принят`, `Отменен`, `Обработан`, `Завершен`. `Просрочен` — производное состояние, в БД не хранится |
| status_color | TEXT | **Legacy-поле.** Больше не используется приложением и интеграционным API. Сохраняется в схеме для совместимости с существующими базами; новое приложение его не записывает и не читает. Цвет строки вычисляется приложением из `status` |
| created_at | DATETIME | Создан |
| updated_at | DATETIME | Изменен |
...
- Повторный выбор того же действия — no-op: `action_set_at` и
  `action_received_at` не изменяются (запись в очередь не возвращается).
- Подтверждение получения 1С — только `action_received_at=now()`
  (действие не меняется), идемпотентно.
- «Отмена» — если запись существует и действие непустое, очищаются
  `action` (пустая строка) и `action_received_at`, обновляется
  `action_set_at=now()`. Если записи нет или действие уже пустое —
  ничего не происходит. Отменённое действие (`action=''`) — обычное
  изменение, ожидающее передачи в 1С.
- Действие доступно только для синхронизированных документов
  (`receipts.uuid IS NOT NULL`).
- Для документа в статусе `Отменен` установка/изменение/отмена действия
  запрещены (UI не показывает кнопку, обработчики отвечают 403). Уже
  установленное до отмены действие при этом сохраняется и передаётся в 1С
  как обычно.
...
# 8. База данных files.db

Отдельная SQLite-база для хранения файлов документов.
Создаётся автоматически при старте из описания таблицы (`receipts.FilesTable`).

## Таблица receipt_files

Назначение: Хранение PDF-файлов документов. Модель допускает расширение
на другие типы файлов без изменения механизма хранения.

| Поле | Тип | Описание |
|------|-----|----------|
| id | INTEGER PK | Первичный ключ |
| receipt_id | INTEGER | Логическая связь → receipts.id (без FK) |
| uuid | TEXT | Внешний UUID файла из 1С, уникален в пределах документа |
| file_name | TEXT | Имя файла |
| mime_type | TEXT | MIME-тип |
| file_size | INTEGER | Размер файла |
| file_data | BLOB | Содержимое файла (без сжатия) |
| created_at | DATETIME | Дата загрузки |
| updated_at | DATETIME | Изменен |

Ограничения:

- `UNIQUE (receipt_id, uuid)` — идемпотентная вставка по паре
  «документ + внешний UUID файла». Один и тот же uuid в разных
  документах может существовать как независимые записи.

Связи:

- ReceiptFile принадлежит одному документу.
- На один документ может быть от нуля до нескольких файлов.
- Внешний ключ между `files.db` и `base.db` отсутствует: SQLite не
  поддерживает межбазовые FK. `receipt_id` — логическая связь.
...
Каждый файл хранится как BLOB без дополнительного сжатия.

Файлы никогда не изменяются после сохранения в интерфейсе.
Файлы могут обновляться через API (полная замена содержимого и
метаданных по внешнему uuid).
Удаление PDF через пользовательский интерфейс не предусмотрено.

На первом этапе API принимает только PDF. Модель и таблица допускают
расширение на другие типы файлов без изменения механизма хранения.
