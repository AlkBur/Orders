...
- «Отмена» — если запись существует, очищаются `action` (пустая строка)
  и `action_received_at`, обновляется `action_set_at=now()`. Если записи
  нет — ничего не происходит. Отменённое действие (`action=''`) — обычное
