...
PDF-файлы хранятся в отдельной базе `files.db` (см. DATABASE.md).
Orders Server отвечает за их хранение и отдаёт пользователю для просмотра.
...
5. **Уведомления Gotify** (`internal/gotify`, `internal/app/notify.go`,
   `internal/app/notify_templates.go`): транспорт вынесен в отдельный пакет,
   не знающий о доменной модели. Настройки — секция `gotify` (`url`,
   `priority`, `tokens[]`); одно уведомление уходит в каждое приложение.
   Два типа сообщений: **status** (любое фактическое изменение статуса:
   UI-отправка, пометка на удаление, статусы Integration API) и **action**
   (фактическое изменение действия). Тексты — отдельные `text/template`
   шаблоны, правятся в `notify_templates.go`. Инициатор (actor) — логин
   сессии, для Integration API — «1С»; автор документа не подставляется.
   Рассылка best-effort и асинхронная: сбой Gotify только логируется и не
   влияет на операцию пользователя. `Store.SetAction` и
   `Store.UpdateByExternal` возвращают признак фактического изменения,
   `Store.SynchronizeByID` — `StatusChanges`, поэтому повторные значения
