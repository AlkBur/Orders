...
5. **Уведомления Gotify** (`internal/gotify`, `internal/app/notify.go`):
   транспорт вынесен в отдельный пакет, не знающий о доменной модели.
   Настройки — секция `gotify` (`url`, `priority`, `tokens[]`); одно
   уведомление уходит в каждое приложение. Рассылка best-effort и
   асинхронная: сбой Gotify только логируется и не влияет на операцию
   пользователя. `Store.SetAction` возвращает `changed`, поэтому уведомление
   отправляется только при фактическом изменении действия.
