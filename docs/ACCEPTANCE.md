...
- [ ] POST `/receipts` с подменённой старой `date` и чужим `user_id` → сохраняются текущая дата и текущий пользователь
- [ ] Копирование документа с чужой датой/автором → новый документ с текущей датой и текущим пользователем
- [ ] Карточка просмотра показывает дату со временем
- [ ] Список показывает пользователя-создателя; отбор по пользователю сужает список
- [ ] У отменённого документа кнопки «Файлы» в списке нет, файлы в карточке не открываются, прямой запрос файлов → 403
