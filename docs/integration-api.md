...
которых установлено текущее действие (`receipt_actions.action`) и действие
ещё не получено 1С (`action_received_at IS NULL`).
...
- `number` — номер документа;
- `date` — дата документа, формат `YYYY-MM-DD`;
- `action` — запрошенное действие, одно из: `Удалить`, `Изменить`.
