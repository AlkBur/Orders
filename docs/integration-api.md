...
- `status` — текст статуса, необязательное. Допустимы только фиксированные
  статусы:
...
не сбрасываются. Для `status` действует семантика по наличию поля:

| Значение | Поведение |
|----------|-----------|
| поле отсутствует (`null`) | не менять |
| `""` | удалить статус |
| `"значение"` | установить/изменить |

Поле `status_color` больше не поддерживается: цвет строки вычисляется
приложением из фиксированного статуса. Передача `status_color`
отклоняется (`400`, неизвестное поле). Статус `Просрочен` передавать
нельзя — это производное состояние, вычисляемое приложением
(StatusCreated старше 24 часов).

Строка без полей пропускается.
...
| Код | Причина |
|-----|---------|
| `400 Bad Request` | Неверный JSON, пустой/отрицательный `id`, пустой `uuid` (`ErrEmptyUUID`), недопустимый `status` (не из фиксированного списка), `status_color` (неизвестное поле), лишние данные после JSON |
| `401 Unauthorized` | X-API-Key отсутствует, пуст или неверен |
...
- `status` — текст статуса, опциональное. Допустимы только фиксированные
  статусы (`Создан`, `Отправлен`, `Принят`, `Отменен`, `Обработан`,
  `Завершен`). Статус `Просрочен` передавать нельзя — это производное
  состояние.

Обновление частичное (PATCH-семантика): заполняются только переданные
поля, остальные не сбрасываются. `status` обязателен. Отсутствие поля
(или `null`) не меняет текущее значение:

- `{"status":"Принят"}` — изменить только статус;
- `{}` — отклоняется (`400`).

Поле `status_color` больше не поддерживается: цвет строки вычисляется
приложением из статуса. Передача `status_color` отклоняется (`400`,
неизвестное поле).
...
| Код | Причина |
|-----|---------|
| `400 Bad Request` | Неверный JSON, пустое тело (нет `status`), недопустимый `status` (не из фиксированного списка), `status_color` (неизвестное поле), лишние данные после JSON |
| `401 Unauthorized` | X-API-Key отсутствует, пуст или неверен |
| `404 Not Found` | Организация не существует; чек с `ruuid` не существует; чек принадлежит другой организации |
| `500 Internal Server Error` | Ошибка базы данных |
...
### 5.7 GET /api/integration/organizations/{oid}/receipts/actions

Очередь запрошенных действий по документам: документы организации, у
которых есть запись в `receipt_actions` с `action_received_at IS NULL`.
Это все изменения, ожидающие передачи в 1С, — установка действия
(`Удалить`/`Изменить`) либо его отмена (пустой `action`).

#### Request

```
GET /api/integration/organizations/{oid}/receipts/actions
X-API-Key: <ключ>
```
...
- `uuid` — внешний UUID документа (назначен 1С при синхронизации);
- `number` — номер документа;
- `date` — дата документа, формат `YYYY-MM-DD`;
- `action` — запрошенное действие: `Удалить`, `Изменить` или пустая
  строка. Пустая строка означает, что ранее установленное действие
  отменено.
...
### 5.8 PUT /api/integration/organizations/{oid}/receipts/actions

Подтверждение получения действий: по внешнему `uuid` документа 1С сообщает,
что информация получена. Устанавливается `action_received_at = now()`,
само действие не изменяется. Операция атомарна для всего массива;
повторное подтверждение идемпотентно.

#### Request

```
PUT /api/integration/organizations/{oid}/receipts/actions
Content-Type: application/json
X-API-Key: <ключ>
```
...
- `uuid` — внешний UUID документа, обязательное, непустое.
...
- `updated` — количество действий, по которым установлена дата получения.

#### Ошибки

| Код | Причина |
|-----|---------|
| `400 Bad Request` | Неверный JSON, пустой `uuid`, неизвестное поле, лишние данные после JSON |
| `401 Unauthorized` | X-API-Key отсутствует, пуст или неверен |
| `404 Not Found` | Организация не существует; документ с `uuid` не найден в этой организации |
| `500 Internal Server Error` | Ошибка базы данных |
