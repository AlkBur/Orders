...
type ReceiptListRow struct {
	...
	// когда оно установлено; иначе пустая строка (действия нет или оно
	// отменено). Отображается независимо от подтверждения 1С.
	Info string

	// InfoPending — действие установлено, но ещё не подтверждено 1С
	// (action_received_at == nil). Влияет только на оформление: такое
	// действие подсвечивается в колонке «Инфо».
	InfoPending bool
